...
namespace GpTimer
{
	...
	void GpTimer::Pwm::config( u32 period_us,
		 					   float duty, 
					 		   float tick_resolution_us,
					 		   GpTimer::config::pwmConfig::outputLevel::_outputLevel pwmOutputLevel )
	{
		this->config( period_us,
					  tick_resolution_us,
					  GpTimer::config::timerConfig::joiningMode::disjoint,
					  GpTimer::config::timerConfig::countDirection::down,
					  GpTimer::config::timerConfig::timerMode::periodic,
					  GpTimer::config::timerConfig::timerInterrupts::none );
		// Disable subtimer
		reinterpret_cast<MapTimer::GPTimerRegs*>
			( this->m_timerMod )->GPTMCTL &= 0xffffffff - ( 1 << this->m_subTimer );
	}
}
